  ...
  const steps = [
    {
      id: 1,
      title: currentMini
        ? (hasSavedPracticeSession(currentBankLabel?.toLowerCase()?.includes('qbank2') ? 'qbank2' : currentBankLabel?.toLowerCase()?.includes('qbank3') ? 'qbank3' : 'qbank1', currentMini)
        ...
        {steps.map((step) => {
          ...
          const content = (
            ...
            if (locked) {
              const lockedReason =
                activeStep === 3
                  ? 'Mini Mock complete. Restart to improve your score.'
                  : step.id === 2
